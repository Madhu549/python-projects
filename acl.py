...
def framing_username_and_password(username):
  for deploy_id in other_config_is['otherConfigIS']['isuser']:
    ...
    return deploy_id[values]['username'], other_config_is['otherConfigIS']['isuserdefault']['password']
...
def invoke_wm_service(RowID, acl_name, acl_service, action, artifact_type, groupname, status, target_server_alias_list, username, pwd_to_create_ISUser, protocol, is_user_name, server):
  ...
  java_Exec = f'{java_home}/bin/java -cp {java_lib_path}wm-isclient.jar:{java_lib_path}vf-wm-release.jar:{java_lib_path}gf.javax.mail.jar:{java_lib_path}enttoolkit.jar invokeService {protocol} "{server}" {is_user_name} {is_password} "{service_to_invoke}" "SprintName|{sprint_name}" "ReleaseName|{release_pipeline_number}" "TargetServerAlias|{target_server_alias_list}" "ArtifactType|{artifact_type}" "Username|{username}" "GroupName|{groupname}" "ACLName|{acl_name}" "RowID|{RowID}" "ACLService|{acl_service}" "Status|{status}" "Action|{action}" "LogFile|{log_file_path}" "Password|{pwd_to_create_ISUser}"'
