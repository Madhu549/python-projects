...
class ACLComparison:
    ...
    def fetch_packages_list(self, type):
        ...
        for package in temporary_list:
            if (os.path.isdir(f"{self.system_working_directory}/extracted_packages_{type}/packages/{package}") == False) or (re.match('^Wm', package)):
                a.append(package)
...
def send_comparison_result_mail(result_data_frame):
    ...
    receipents = ['madhubabu.tammu@vodafone.com', 'jakira.shaik@vodafone.com']
    #receipents = []
    #for i in email_config[pipeline_type]:  #flattening lists
    #    receipents.extend(i)
    cc = ['madhubabu.tammu@vodafone.com', 'jakira.shaik@vodafone.com']
    # cc = []
...
def acl_comaprison():
    ...
    try:
        ...
        for source_and_target_alias in source_and_target_alias_list:
            ...
            for package_name in packages_list:                
                ...
                if source_packages_status_list[-1] == 'Enabled' and target_packages_status_list[-1] == 'Enabled':
                    ...
                    source_packages_status_list.pop()
                    source_packages_exist_list.pop()
                    target_packages_status_list.pop()
                    target_packages_exist_list.pop()

                    print('acl elements has been fetched successfully for source package and target package')
                    print(f'length of source_acl_element_list: {len(source_acl_element_list)} and length target_acl_element_list: {len(target_acl_element_list)}')

                    if len(source_acl_element_list)==0 and len(target_acl_element_list)==0:
                        ...
                        source_packages_status_list.append('Enabled')
                        source_packages_exist_list.append('Yes')
                        target_packages_status_list.append('Enabled')
                        target_packages_exist_list.append('Yes')
                    ...
                    elif len(source_acl_element_list)>0 and len(target_acl_element_list)==0:
                        ...
                        for i in range(len(target_acl_element_list)):
                            source_environment_list.append(source_environment)
                            target_environment_list.append(target_environment)
                            zone_list.append(comparison_object.source_alias_list[1])
                            instance_list.append(comparison_object.source_alias_list[2])
                            ...
                            target_packages_exist_list.append('Yes')
                            source_system_acl_element.append('No-ACLs-tagged')
                            target_system_acl_element.append(target_acl_element_list[i])
                            source_acl_tagged.append('No-ACLs-tagged')
                            target_acl_tagged.append(target_acl_value_list[i])
                            acl_match_list.append('ACL-mis-matched')
                    else:
                        if len(source_acl_element_list)>len(target_acl_element_list):
                            for i in range(len(source_acl_element_list)):
                                source_environment_list.append(source_environment)
                                target_environment_list.append(target_environment)
                                zone_list.append(comparison_object.source_alias_list[1])
                                ...
                                try:
                                    ...
                                    target_system_acl_element.append('ACL-element-does not-exist')
                                    target_acl_tagged.append('ACL-value-does not-exist')                            
                            ...
                            for i in range(len(target_acl_element_list)):
                                ...
                                try:
                                    ...
                                    source_system_acl_element.append('ACL-element-does not-exist')
                                    source_acl_tagged.append('ACL-value-does not-exist')                            
                ...
                elif source_packages_status_list[-1] == 'Disabled' or 'NA' and target_packages_status_list[-1] == 'Disabled' or 'NA':
                    source_packages_status_list.pop()
                    source_packages_exist_list.pop()
                    target_packages_status_list.pop()
                    target_packages_exist_list.pop()
            ...
            print(f'packages lenth:{len(packages_list)}')
            print(len(target_environment_list))
            print(len(zone_list))
            print(len(instance_list))
            print(len(package_name_list))
            print(f'source package exist list: {len(source_packages_exist_list)}')
            print(f'target package exist list: {len(target_packages_exist_list)}')
            print(len(source_system_acl_element))
            print(len(target_system_acl_element))
            print(len(source_acl_tagged))
            print(len(target_acl_tagged))
            print(len(acl_match_list))
            print('lengths before no manifest')
            packages_list = no_manifest_packages_source[:]
            packages_list.extend(no_manifest_packages_target)
            packages_list = list(set(packages_list))
            packages_list.sort()
            
            print(f'no manifest length: {len(packages_list)}')
            ...
            print('lengths after manifest')
            print(len(source_environment_list))
            print(len(target_environment_list))
            print(len(zone_list))
            print(len(instance_list))
            print(len(package_name_list))
            print(len(source_packages_exist_list))
            print(len(target_packages_exist_list))
            print(len(source_packages_status_list))
            print(len(target_packages_status_list))
            print(len(source_system_acl_element))
            print(len(target_system_acl_element))
            print(len(source_acl_tagged))
            print(len(target_acl_tagged))
        ...
        result_data_frame = pd.DataFrame({'Source Environment': source_environment_list, 'Target Environment': target_environment_list, 'Zone': zone_list, 'Instance': instance_list, 'Package name': package_name_list, 'Package Exists - Source': source_packages_exist_list, 'Package Exists - Target': target_packages_exist_list,'Source package status list': source_packages_status_list, 'Target package status list': target_packages_status_list, 'Source-system-ACL-Element': source_system_acl_element, 'Target-system-ACL-Element': target_system_acl_element, 'Source-ACL-tagged': source_acl_tagged, 'Target-ACL-tagged': target_acl_tagged, 'ACL_match_list': acl_match_list}, dtype = str)    
