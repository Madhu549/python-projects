...
def checking_target_server_alias(target_alias_name):
  try:
    for target_alias in is_config_dict[target_alias_name[0]].keys():
      target_server_alias_name = f'{env}_{target_alias}'
      return target_server_alias_name
  except KeyError:
    print(f'{target_alias_name[0]} is not present in {env} config file\n')
...
def framing_acl_data():
      ...
      for id in build_config[otherisconfig]:
        for values in id.keys():
          ...
          try:
            ...
            for alias in target_server_list1:
  ...
  print(ACL_NAME, ACL_SERVICE, ACTION, ARTIFACT_TYPE, ERROR_MESSAGE, FOR_DEPLOYMENT, GROUPNAME, ROLLBACK_ID, STATUS, target_server_list, USERNAME, pwd_to_create_ISUser)
...
if __name__=='__main__':
    try:
      ...
      print(acl_name, acl_service, action, artifact_type, error_message, for_deployment, groupname, rollback_id, status, target_server_alias, username, pwd_to_create_ISUser, rollbackId, protocol, is_user_name, server)
