...
    def fetch_version_status(self,package_name):

        for path, dirnames, filenames in os.walk(package_name):
...
                    status = my_dict["Values"]["value"][0]["@name"]

                    self.package_dict[file_name]=[version,status]

                else:

                    print(f'manifest.v3 file does not exists in {file_name}')
...
        for package_name in old_packages_list:

            comparison_result = comparison_object.perform_package_compaison(package_name)

            old_package_dictionary = comparison_object.fetch_version_status(package_name)

            new_package_dictionary = comparison_object.fetch_version_status(package_name)

            package_names.append(package_name)

            comparison_results.append(comparison_result)

            old_status_list.append(old_package_dictionary[package_name][1])

            new_status_list.append(new_package_dictionary[package_name][1])

            old_version_list.append(old_package_dictionary[package_name][0])

            new_version_list.append(new_package_dictionary[package_name][0])

        

        result_data_frame = pd.DataFrame({'source-Package-name' : package_name, 'source-package-status' : old_status_list, 'target-package-status' : new_status_list, 'source-package-version' : old_version_list, 'target-package-version' : new_version_list }, dtype = str)
